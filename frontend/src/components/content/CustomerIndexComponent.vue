		...
		<tbody>
			<div v-for="(item, index) in this.data.data">
				<tr>
					...
					<td>
						<div class="d-flex justify-content-center">
							<router-link :to='{name:"customerform", params: {status: "edit", index: item.id}, }' class="btn btn-warning btn-sm">
								Edit
							</router-link>
						</div>
					</td>
...
export default {              
  ...
  methods:{
	async loaddata(){
		let urllink = `http://127.0.0.1:4000/customer/index`;
